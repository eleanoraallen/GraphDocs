...
import {QueryString, MutationString, SubscriptionString, OperationString} from './query_strings';
...
const ReferenceLink = (window.location.href.split('/?page=')[0].replace('http://','') + 
  '/?page=referenceRequest=printType:NAME').replace('//?page', '/?page');
...
function printFilteredOperations(operationType, include, exclude) {
  ...
  return <Query query={gql(QueryString)}>
    {({ loading, data, error}) => {
      if (loading) return(<p>Loading Queries...</p>);
      if (error) return(<p>Error Loading Queries!</p>);
      if (data) return printTable(filterOperations(data.__schema.queryType.fields, include, exclude),
        data.__schema.types, 'Query'); }}
    </Query>;
  } if (operationType === 'mutation') {
    return <Query query={gql(MutationString)}>
      {({ loading, data, error}) => {
        if (loading) return(<p>Loading Mutations...</p>);
        if (error) return(<p>Error Loading Mutations!</p>); 
        if (data) return printTable(filterOperations(data.__schema.mutationType.fields, include, exclude), 
          data.__schema.types, 'Mutation'); }}
      </Query>;
  } if (operationType === 'subscription') {
    return <Query query={gql(SubscriptionString)}>
      {({ loading, data, error}) => {
        if (loading) return(<p>Loading Subscriptions...</p>);
        if (error) return(<p>Error Loading Subscriptions!</p>); 
        if (data) return printTable(filterOperations(data.__schema.subscriptionType.fields, include, exclude),
          data.__schema.types, 'Subscription'); }}
      </Query>;
  } else {
    return <Query query={gql(OperationString)}>
      {({ loading, data, error}) => {
        if (loading) return(<p>Loading Operations...</p>);
        if (error) return(<p>Error Loading Operations!</p>); 
        if (data) return printTable(filterOperations( data.__schema.queryType.fields.concat(
          data.__schema.mutationType.fields).concat(data.__schema.subscriptionType.fields), include, exclude),
          data.__schema.types, 'Operation'); }}
    </Query>;
  }
}
...
function filterOperations(operations, include, exclude) {
  ...
  return operations.filter(operation => {
    ...
    checkedInclude = checkedInclude || 
      includeList.map(include => operation.name.toLowerCase().includes(include)).reduce((x,y)=> x || y, false);
    checkedExclude = checkedExclude || 
      excludeList.map(exclude => operation.name.toLowerCase().includes(exclude)).reduce((x,y)=> x || y, false);
    // Check Operation Args
    checkedInclude = checkedInclude || 
      operation.args.map(arg => includeString.includes(arg.name.toLowerCase()) || 
        includeString.includes(getType(arg).toLowerCase())).reduce((x,y)=> x || y, false);
    checkedExclude = checkedExclude || 
      operation.args.map(arg => excludeString.includes(arg.name.toLowerCase()) || 
        excludeString.includes(getType(arg).toLowerCase())).reduce((x,y)=> x || y, false);
    // Check Operation Type
    checkedInclude = checkedInclude || 
      includeList.map(include => getType(operation).toLowerCase().includes(include)).reduce((x,y)=> x || y, false);
    checkedExclude = checkedExclude || 
      excludeList.map(exclude => getType(operation).toLowerCase().includes(exclude)).reduce((x,y)=> x || y, false);
...
function printTable(operations, types, tableName) {
  const tableString = `| ${tableName} | Description |\n| - | - |\n` + 
  operations.filter(operation => operation.name !== "temp__").map(operation => {
      try { 
        return `| **\`${operation.name}\`** ${stringifyArgs(operation.args, types)}\`:\`&nbsp; ` + 
        `${stringifyOutput(operation.type, types)} &nbsp;  | *${operation.description}*  |\n`;
    } catch {}}).sort().join('');
  return(<ReactMarkdown source={tableString} 
    renderers={
...
function stringifyArgs(args, types) {
  if (args.length > 1) {
    const last = `*\`${args[args.length - 1].name}: \`* ${stringifyType(getType(args[args.length - 1]), types)}*\`)\`*`;
    args.pop();
    return( '`(`' + 
      args.map(arg => { return `*\` ${arg.name}:\`* ${stringifyType(getType(arg), types)},&nbsp;&nbsp;`; }).join('') + 
      last);
  } if (args.length === 1) {
    return `\`(\`*\`${args[0].name}: \`* ${stringifyType(getType(args[0]), types)}*\`)\`*`;
  } else {
    return '';
  }
}
...
export default class OperationTableComponent extends Component {
  ...
  return (
      ...
      {printFilteredOperations(this.props.operationType.toLowerCase(), this.props.include, this.props.exclude)}
      </ApolloProvider>
    );
  }
}